    ...
    S: Fn(Runnable),
{
    ...
    unsafe fn wake(ptr: *const ()) {
        ...
        loop {
            ...
            if state & SCHEDULED != 0 {
                match (*raw.header).state.compare_exchange_weak(state,state,Ordering::AcqRel,Ordering::Acquire) {
                    Ok(_) => {
                        Self::drop_waker(ptr);
                        break;
                    },
                ...
                match (*raw.header).state.compare_exchange_weak(state,state | SCHEDULED,Ordering::AcqRel,Ordering::Acquire) {
                    Ok(_) => {
                        if state & RUNNING == 0 {
                            Self::schedule(ptr);
                        }
    ...
    unsafe fn wake_by_ref(ptr: *const ()) {
        ...
        loop {
            ...
            if state & SCHEDULED != 0 {
                ...
                match (*raw.header).state.compare_exchange_weak(state,new,Ordering::AcqRel,Ordering::Acquire) {
                    Ok(_) => {
                        if state & RUNNING == 0 {
    ...
    unsafe fn drop_waker(ptr: *const ()) {
        ...
        if new & !(REFERENCE - 1) == 0 && new & TASK == 0 {
            if new & (COMPLETED | CLOSED) == 0 {
                (*raw.header).state.store(SCHEDULED | CLOSED | REFERENCE, Ordering::Release);
                Self::schedule(ptr);
            }
            else {
                Self::destroy(ptr);
    ...
    unsafe fn run(ptr: *const ()) -> bool {
        ...
        struct Guard<F,T,S>(RawTask<F,T,S>) where F: Future<Output = T>,S: Fn(Runnable);

        impl<F,T,S> Drop for Guard<F,T,S> where F: Future<Output = T>,S: Fn(Runnable) {
            fn drop(&mut self) {
                let raw = self.0;
                ...
                unsafe {
                    ...
                    loop {
                        if state & CLOSED != 0 {
                            ...
                            (*raw.header).state.fetch_and(!RUNNING & !SCHEDULED, Ordering::AcqRel);
                            let mut awaiter = None;
                            if state & AWAITER != 0 {
                                awaiter = (*raw.header).take(None);
                            }
                        ...
                        match (*raw.header).state.compare_exchange_weak(state,(state & !RUNNING & !SCHEDULED) | CLOSED,Ordering::AcqRel,Ordering::Acquire) {
                            Ok(state) => {
                                RawTask::<F,T,S>::drop_future(ptr);
                                let mut awaiter = None;
                                if state & AWAITER != 0 {
