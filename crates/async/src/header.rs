...
impl Header {
    ...
    pub(crate) fn take(&self,current: Option<&Waker>) -> Option<Waker> {
        ...
        if state & (NOTIFYING | REGISTERING) == 0 {
            ...
            self.state.fetch_and(!NOTIFYING & !AWAITER, Ordering::Release);
            if let Some(w) = waker {
                match current {
                    None => return Some(w),
    ...
    pub(crate) fn register(&self,waker: &Waker) {
        ...
        loop {
            ...
            match self.state.compare_exchange_weak(state,state | REGISTERING,Ordering::AcqRel,Ordering::Acquire) {
                Ok(_) => {
                    state |= REGISTERING;
                    break;
                },
        ...
        loop {
            ...
            match self.state.compare_exchange_weak(state, new, Ordering::AcqRel, Ordering::Acquire) {
                Ok(_) => break,
                Err(s) => state = s,
            }
        }
