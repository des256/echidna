...
impl Event {
    ...
    fn inner(&self) -> &Inner {
        ...
        if inner.is_null() {
            ...
            inner = self.inner.compare_exchange(inner,new,Ordering::AcqRel,Ordering::Acquire).unwrap_or_else(|x| x);
            if inner.is_null() {
                inner = new;
            }
            else {
        ...
        loop {
            match deadline {
                ...
                Some(deadline) => {
                    let now = Instant::now();
                    if now >= deadline {
                        return self.inner.lock().remove(entry,self.inner.cache_ptr()).is_notified();
                    }
