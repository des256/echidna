...
impl<T> Single<T> {
    ...
    pub fn push(&self,value: T) -> Result<(),PushError<T>> {
        let state = self.state.compare_exchange(0,LOCKED | PUSHED,Ordering::SeqCst,Ordering::SeqCst).unwrap_or_else(|x| x);
        if state == 0 {
    ...
    pub fn pop(&self) -> Result<T,PopError> {
        ...
        loop {
            let prev = self.state.compare_exchange(state,(state | LOCKED) & !PUSHED,Ordering::SeqCst,Ordering::SeqCst).unwrap_or_else(|x| x);
            if prev == state {
